...
namespace EasyTranslate.Translators
{
    internal class TokenGenerator
    {
            ...
            for (var i = 0; i < token.Length; i++)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        if (55296 == (e & 64512) &&
                            i + 1 < token.Length &&
                            56320 == (ASCIICode(token.Substring(i + 1)) & 64512))
                        {
        ...
        private static int ASCIICode(string str) => Convert.ToInt32(Convert.ToString(str[0], 10));

        private static long Xi(long a, string b)
        {
            for (var i = 0; i < b.Length - 2; i += 3)
            {
                string c = b.Substring(i + 2, 1);
                int d = string.CompareOrdinal("a", c) <= 0 ? ASCIICode(c) - 87 : Convert.ToInt32(c); // "a" <= evenChar
